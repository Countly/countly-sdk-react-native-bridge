...
function loadCountlyBridge({ platform = "ios", nativeOverrides = {} } = {}) {
    ...
    jest.doMock("react-native/Libraries/Core/Devtools/parseErrorStack.js", () => jest.fn(() => []));
    ...
    function emit(eventName, payload) {
        listeners
                .filter((listener) => listener.eventName === eventName && !listener.removed)
...
test("initWithConfig serializes 26.1.0 init options and wires the global content callback", async () => {
    ...
    config.content
        .setGlobalContentCallback(contentCallback)
        .setWebViewDisplayOption(Countly.webViewDisplayOption.SAFE_AREA);
...
test("session, device, and content bridge calls keep their caller order", async () => {
    ...
    Countly.endSession();
    Countly.changeDeviceId("device-2", true);
    ...
    expect(nativeModule.changeDeviceId).toHaveBeenCalledWith(["device-2", "1"]);
    ...
    const callOrder = [
        ...
        nativeModule.changeDeviceId.mock.invocationCallOrder[0],
...
test("addCustomNetworkRequestHeaders skips invalid values and stringifies the rest", async () => {
    ...
    expect(nativeModule.addCustomNetworkRequestHeaders).toHaveBeenCalledWith([
        "Authorization",
        "Bearer token",
        "attempts",
        "3",
...
test("feedback.showNPS selects the matching widget and removes the close listener after callback", async () => {
    const widgets = [
        ...
        { id: "nps-2", type: "nps", name: "Priority NPS", tags: ["vip"] },
    ];
    ...
    expect(nativeModule.presentFeedbackWidget).toHaveBeenCalledWith(["nps-2", "nps", "Priority NPS", ""]);
